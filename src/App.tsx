...
import { Language, ActivePage, User, Sitter, Booking, Message, Pet, Review } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const fireSitters = await getSitters();
        setSitters(fireSitters);

        const fireBookings = await getBookings();
        setBookings(fireBookings);
  ...
  useEffect(() => {
    if (currentSitterUser) {
      localStorage.setItem('amuch_sitter_user', JSON.stringify(currentSitterUser));
    } else {
  ...
  useEffect(() => {
    if (selectedSitter) {
      localStorage.setItem('amuch_selected_sitter', JSON.stringify(selectedSitter));
    } else {
  ...
  useEffect(() => {
    ...
    let metaDesc = "Trouvez un pet sitter de confiance au Maroc. Services de garde vérifiés pour chiens et chats.";

    if (language === 'AR') {
      pageTitle = "أموش — ابحث عن حارس حيوانات موثوق في المغرب";
      metaDesc = "أموش يربط أصحاب الحيوانات الأليفة بحراس حيوانات معتمدين وموثوقين في المغرب. احجز الآن وسافر بدون قلق.";
    } else if (language === 'EN') {
      pageTitle = "AMUCH — Find a Trusted Pet Sitter in Morocco";
      metaDesc = "AMUCH connects pet owners with verified pet sitters in Morocco. Book online, live chat and travel worry-free.";
    }
  ...
  const handleBookSitterFlow = (sitter: Sitter, dates: { start: string; end: string }) => {
  ...
  const handleConfirmBooking = () => {
    ...
    const pet = currentUser.pets.find(p => p.id === bookingPetId) || currentUser.pets[0];
    ...
    setBookingModal({ isOpen: false, sitter: null, dates: { start: '', end: '' } });
      ...
      <div className="flex-grow">
        ...
        {activePage === 'search' && (
          <Search
            ...
            onBookSitter={(sitter, dates) => handleBookSitterFlow(sitter, dates)}
          />
        )}
        ...
        {activePage === 'signup-owner' && (
          <SignUpOwner
            ...
            onLoginComplete={(user) => {
              setCurrentUser(user);
              if (user.role === 'sitter') {
                const matched = sitters.find(s => s.id === user.id) || sitters[0];
        ...
        {activePage === 'owner-dashboard' && currentUser && (
          <OwnerDashboard
            ...
            bookings={bookings.filter((b) => b.ownerId === currentUser.id || b.ownerId === 'owner-default')}
            chats={[]}
        ...
        {activePage === 'sitter-dashboard' && currentSitterUser && (
          <SitterDashboard
            ...
            onLogout={handleLogout}
            bookings={bookings.filter((b) => b.sitterId === currentSitterUser.id || b.sitterId === 'sitter-1')}
            onUpdateBookingStatus={handleUpdateBookingStatus}
            onNavigateToChat={() => setActivePage('chat')}
            reviews={reviews.filter(r => r.sitterId === currentSitterUser.id || r.sitterId === 'sitter-1')}
          />
        )}
          ...
          >
            ...
            <div className="bg-[#FF6B00] text-white p-5 font-bold text-center">
              <h3 className="text-lg">
                {language === 'FR' ? "Finaliser la Demande de Garde" : "Confirm Booking Request"}
              </h3>
              <p className="text-xs text-white/90 font-semibold mt-1">
                {bookingModal.sitter.firstName} {bookingModal.sitter.lastName} &bull; {bookingModal.sitter.city}
              </p>
            </div>
            ...
            <div className="p-6 space-y-5 text-sm">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-gray-400 uppercase tracking-wide block">
                  {language === 'FR' ? "Quel compagnon faire garder ?" : "Which pet requires care?"}
                </label>
                <select
                  value={bookingPetId}
                ...
                >
                  {currentUser.pets.map((p) => (
                    <option key={p.id} value={p.id}>{p.photoUrl} {p.name} ({p.breed})</option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 bg-gray-50 p-3 rounded-xl text-center">
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase block">{language === 'FR' ? "Arrivée" : "Start"}</span>
                  <span className="font-extrabold text-xs text-[#111111]">{bookingModal.dates.start}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold text-gray-400 uppercase block">{language === 'FR' ? "Départ" : "End"}</span>
                  <span className="font-extrabold text-xs text-[#111111]">{bookingModal.dates.end}</span>
                </div>
              </div>

              {/* Pricing breakdown summary */}
              <div className="flex justify-between items-baseline pt-2 border-t border-gray-100 font-bold">
                <span className="text-gray-500">{language === 'FR' ? "Tarif nuit" : "Rate per night"}</span>
                <span className="text-sm text-[#111111]">{bookingModal.sitter.pricePerNight} MAD</span>
              </div>
              ...
              <div className="flex justify-between items-baseline pt-1 font-bold">
                ...
                <span className="text-xl text-[#FF6B00]">
                  {Math.max(1, Math.round((new Date(bookingModal.dates.end).getTime() - new Date(bookingModal.dates.start).getTime()) / (1000 * 3600 * 24))) * bookingModal.sitter.pricePerNight} MAD
                </span>
              </div>
            </div>
            ...
            <div className="bg-gray-50 px-6 py-4 flex gap-3 border-t border-gray-100">
              ...
              <button
                onClick={() => setBookingModal({ isOpen: false, sitter: null, dates: { start: '', end: '' } })}
